 ...
 **/
	...
	class db{
		...
		public function getSqlData_ok($sql,$type=MYSQL_NUM){
			$arr = array();//定义一个数组
			// $i=0;//二维数组行下标
			$query = mysql_query($sql);
			while($res=mysql_fetch_array($query,$type))
...
/////////////////////////图片更新//////////////////////////////////
	...
	/**
		  ...
		  *    date：2018.7.24
		  */
	public function common($table,$limit)
	{
		$sql = 'select name,ask_type,zan from '.$table.' order by id desc limit '.$limit;
		$query = mysql_query($sql);
		while($res = mysql_fetch_assoc($query))
		{
			$data[] = $res['name'];
			$data[] = $res['ask_type']==0?"<strong style='color:red;'>技术问题</strong>":($res['ask_type']==1?"<strong style='color:blue;'>咨询问题</strong>":"<strong style='color:yellow;'>生活问题</strong>");
			$data[] = $res['zan'];
		}
		return $data;
	}
